...
df_OTP = pd.read_csv('../Ratios/Otp_ratio_neg_1.csv', sep=';')
df_RATIO = pd.read_csv('../Ratios/ratio.csv', sep=';')

x_RATIO= df_RATIO[['Positive','Negative']]
y1_OTP=df_OTP['rateRatio']

# Visualisation RATIO-OTP
...
y_pred = np.linspace(0, 2, 30)  # range of OTP values
...
ax1.text2D(0.2, 0.32, 'OTP', fontsize=13, ha='center', va='center',
           transform=ax1.transAxes, color='grey', alpha=0.5)
ax2.text2D(0.3, 0.42, 'OTP', fontsize=13, ha='center', va='center',
           transform=ax2.transAxes, color='grey', alpha=0.5)
ax3.text2D(0.85, 0.85, 'OTP', fontsize=13, ha='center', va='center',
...
fig.suptitle('OTP-RATIO $R^2 = %.2f$' % r2, fontsize=20)
...
ax_single.text2D(0.3, 0.42, 'OTP', fontsize=13, ha='center', va='center',
           transform=ax_single.transAxes, color='grey', alpha=0.5)

for ii in np.arange(0, 360, 1):
    ax_single.view_init(elev=16, azim=ii)
    fig2.savefig('OTP_gif_image%d.png' % ii)